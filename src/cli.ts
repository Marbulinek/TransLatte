...
program
  .name('translatte')
  .description(
    'TransLatté - Automatic translation file generator for Angular ngx-translate',
  )
  .version('1.0.0');
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (fs.existsSync(configPath)) {
        ...
        spinner.fail(
          'Configuration file not found and required options not provided',
        );
        console.log(chalk.yellow('\nPlease either:'));
        console.log(
          chalk.yellow('1. Create a translatte.config.json file, or'),
        );
        console.log(
          chalk.yellow(
            '2. Provide all required options: --source, --targets, --input, --output\n',
          ),
        );
        process.exit(1);
      }
      ...
      if (!config.sourceLanguage || !config.targetLanguages) {
        spinner.fail(
          'Invalid configuration: sourceLanguage and targetLanguages are required',
        );
        process.exit(1);
      }
      ...
      if (!hasSingleSource && !hasMultipleSources) {
        spinner.fail(
          'Invalid configuration: provide either inputFile/outputDir or sources array',
        );
        process.exit(1);
      }
      ...
      console.log(
        chalk.gray(`   Target Languages: ${config.targetLanguages.join(', ')}`),
      );

      if (hasMultipleSources) {
        console.log(
          chalk.gray(`   Translation Sources: ${config.sources!.length}`),
        );
      ...
      if (hasMultipleSources) {
        const sourceNames = [
          ...new Set(summary.results.map((r) => r.sourceName)),
        ];

        sourceNames.forEach((sourceName) => {
          const sourceResults = summary.results.filter(
            (r) => r.sourceName === sourceName,
          );
          console.log(chalk.cyan(`\n📦 ${sourceName}:`));
          ...
          sourceResults.forEach((result) => {
            if (result.success) {
              console.log(
                chalk.green(
                  `   ✓ ${result.language}.json - Successfully translated`,
                ),
              );
            } else {
              console.log(
                chalk.red(
                  `   ✗ ${result.language}.json - Failed: ${result.error}`,
                ),
              );
            }
          });
        ...
        summary.results.forEach((result) => {
          if (result.success) {
            ...
            console.log(
              chalk.red(`✗ ${result.language}.json - Failed: ${result.error}`),
            );
          }
        });
      }
      ...
      if (summary.successCount > 0) {
        ...
        spinner.succeed(
          chalk.green(
            `Translation completed! ${summary.successCount}/${totalFiles} file(s) generated`,
          ),
        );
        ...
        if (summary.cacheSize !== undefined && summary.cacheSize > 0) {
          console.log(
            chalk.gray(
              `💾 Cache: ${summary.cacheSize} translation(s) stored for faster future runs`,
            ),
          );
        }
      }
      if (summary.failCount > 0) {
        spinner.warn(
          chalk.yellow(`${summary.failCount} translation(s) failed`),
        );
      }
    } catch (error: any) {
...
program
  ...
  .option(
    '-m, --multiple',
    'Create config with multiple sources for modular apps',
  )
  .action((options) => {
    ...
    if (options.multiple) {
      ...
      console.log(
        chalk.green('✓ Created translatte.config.json (multi-module)'),
      );
      console.log(
        chalk.gray('\nConfiguration created for multiple translation sources.'),
      );
      console.log(
        chalk.gray('Edit the sources array to match your module structure.\n'),
      );
    } else {
      ...
      console.log(
        chalk.gray('For multi-module apps, use: translatte init --multiple\n'),
      );
    }
